...
const UpdateProducts = () => {
    ...
    const handleUpdateProduct = async (e, productId) => {
        ...
        try {
            await dispatch(updateProduct({ productId, updatedData: { productname, price, category } }));
                    ...
                    {products.length > 0 ? (
                        <div>
                            {products.map((product) => (
                                <div key={product.productid} className={`${styles['product-container']} ${selectedProductId === product.productid ? styles.open : ''}`}>
                                    <div className={styles['product-info']}>
                                        <span className={styles['product-name']}>
                                            {product.productname}
                                        </span>
                                        <button
                                            className={`${styles['toggle-button']} ${selectedProductId === product.productid ? styles.red : ''}`}
                                    ...
                                    {selectedProductId === product.productid && (
                                        <form onSubmit={(e) => handleUpdateProduct(e, product.productid)} className={styles.updateForm}>
